...
async function ensurePlaywright() {
  try {
    ...
    throw new Error(
      'No se encontro playwright. Ejecuta: npm install ; npx playwright install chromium',
    );
  }
}
...
async function run() {
  ...
  try {
    ...
    exitCode = 1;
    process.stderr.write(`\nError al generar screenshot mock: ${error instanceof Error ? error.message : String(error)}\n`);
  } finally {
    server.kill('SIGTERM');
